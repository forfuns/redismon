...
class RedisFalconMonitor(object):
    ...
    def __init__(self, addr, port, password, cluster_name, endpoint):
        self.addr = addr
        self.port = port
    ...
    def ping_redis(self):
        ...
        if redis_is_alive == 0:  # If redis is dead, update the alive metrice here.
            redis_alive_data = [
                {"endpoint": self.endpoint, "metric": "redis_alive", "tags": self.tags, "timestamp": upload_ts,
...
def main():
    ...
    for redis_ins in redis_items:
        ...
        redis_falcon_monitor = RedisFalconMonitor(redis_address, redis_port, redis_password, redis_clusterName,
                                                  endpoint)
